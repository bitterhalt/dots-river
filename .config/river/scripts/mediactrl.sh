...
show_music_notification() {
  ...
  if [[ "$status" == "Playing" ]]; then
    ...
    notify-send -t 5000 -a playerctrl_notify "Playing:" "$song_title\nby $song_artist" -h string:x-canonical-private-synchronous:volume
  elif [[ "$status" == "Paused" ]]; then
    notify-send -t 5000 -a playerctrl_notify "Playback Paused" -h string:x-canonical-private-synchronous:volume
  fi
}
